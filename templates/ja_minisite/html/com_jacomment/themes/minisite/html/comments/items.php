...
<ol class="comment-list comment-list-lv<?php echo $items[0]->level; ?>">	
	<?php foreach($items as $item): ?>
		...
		<!-- A ROW COMMENT -->
		<li id="jac-row-comment-<?php echo $item->id?>" class="clearfix jac-row-comment <?php echo $jacRow; ?> list-item <?php if($ischild){ if($isEnableThreads){ echo "comment-hasreply";}else{ echo "comment-notree";}}?> rank-high <?php if($k == 0) echo "jac-first ";if($ischild) echo " comment-replycontent";?>">
		...
		<div id="jac-content-of-comment-<?php echo $item->id?>" class="comment-contentmain comment-contentholder clearfix ja-imagesize<?php echo $avatarSize;?> <?php if($item->isSpecialUser){ echo " comment-byadmin";}else{if($item->isCurrentUser) echo " comment-byyou";}if($item->type == 0) echo " comment-ispending"; ?>">						
	        ...
	        <div class="avatar clearfix">                	
				...
				echo '<a href="#jacommentid:'.$item->id.'">'.'<span class="comment-date">'. $item->date .'</span></a>';
				}
	          ?>	          
					...
					<fieldset class="fieldset legend" id="jac-attach-file-<?php echo $item->id;?>">
							...
							<legend><?php echo JText::_("Attached File");?></legend>
							<div id="jac-list-attach-file-<?php echo $item->id;?>" class='jac-list-upload-title'>						
								<?php 																		
									foreach ($listFiles as $listFile) {																
										...
										echo "<img src='". JURI::root() ."/components/com_jacomment/themes/" . $theme . "/images/". $type .".gif' alt='". $listFile ."'/> <a href='". JRoute::_($linkOfFile) ."' title='". JText::_("Download File") ."'>". $listFile ."</a><br />";	
									}																
								?>
							</div>						
					</fieldset>	
	          ...
	          <?php if($item->isAllowVote){
						...
						<?php }	?><span class="vote-comment-<?php echo $avatarSize;?> comment-rankingresult" id="voted-of-<?php echo $item->id;?>"><?php echo $item->totalVote;?> <?php echo JText::_("");?></span>
	          </div>							
			...
			<span class="comment-order">0<?php echo $k+1; ?></span>
		</div>
